...
class ProductInfoViewModel{
    ...
    func isAddedToCart(product:Product) -> Bool{
        ...
        let product = MyCartItems.cartItemsCodableObject?.filter({ $0.variantId == $0.variantId })
        if product?.count == 0{
            return false
        }
        return true
    ...
    func addToCart(draftOrdrId:Int,product:Product){
        ...
        func addItem(product:Product){
            ...
            let item = LineItems(variantId: variantId, title: title ?? "", varientTitle: variTitle ?? "", price: price ?? "", properties: properties, quantity: 1)
